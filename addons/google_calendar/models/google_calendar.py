...
class SyncEvent(object):
    ...
    def compute_OP(self, modeFull=True):
        #If event are already in Gmail and in Cnmx
        ...
        # New in Cnmx...  Create on create_events of synchronize function
        elif self.OE.found and not self.GG.found:
            if self.OE.status:
                self.OP = Delete('OE', 'Update or delete from GOOGLE')
            else:
                if not modeFull:
                    self.OP = Delete('GG', 'Deleted from Cnmx, need to delete it from Gmail if already created')
                else:
                    self.OP = NothingToDo("", "Already Deleted in gmail and unlinked in Cnmx")
...
class GoogleCalendar(models.AbstractModel):
    ...
    def create_an_event(self, event):
        """ Create a new event in google calendar from the given event in Cnmx.
    ...
    def update_from_google(self, event, single_event_dict, type):
        """ Update an event in Cnmx with information from google calendar
    ...
    def update_events(self, lastSync=False):
        ...
        if lastSync:
            ...
            # Select all events from Cnmx which have been already synchronized in gmail
