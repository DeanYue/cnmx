...
#
#
# This program will read an email from stdin and forward it to cnmx. Configure
...
def main():
    ...
    op.add_option("-d", "--database", dest="database", help="Cnmx database name (default: %default)", default='odoo')
    op.add_option("-u", "--userid", dest="userid", help="Cnmx user id to connect with (default: %default)", default=1, type=int)
    op.add_option("-p", "--password", dest="password", help="Cnmx user password (default: %default)", default='admin')
    op.add_option("--host", dest="host", help="Cnmx host (default: %default)", default='localhost')
    op.add_option("--port", dest="port", help="Cnmx port (default: %default)", default=8069, type=int)
