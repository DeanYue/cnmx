...
class MailBot(models.AbstractModel):
    ...
    def _get_answer(self, record, body, values, command):
        ...
        if self._is_bot_in_private_channel(record):
            if cnmxbot_state == "onboarding_ping" and self._is_bot_pinged(values):
                self.env.user.cnmxbot_state = "onboarding_canned"
                return _("That's me! 🎉<br/>Try to type \":\" to use canned responses.")
            elif cnmxbot_state == "onboarding_canned" and values.get("canned_response_ids"):
            ...
            elif cnmxbot_state == 'onboarding_canned':
